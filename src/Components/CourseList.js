...
const CourseList = ({ courses }) => {
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10 justify-center items-center w-full px-5 py-10">
      {courses.map(course => (
        <div key={course.id} className="flex flex-col border p-4 rounded shadow-lg shadow-black">
          <div>
          ...
          <div className="flex flex-col mt-5">
            ...
            <div className="mt-5 flex justify-center">
              <button className="bg-yellow-600 p-4 text-xl font-semibold hover:bg-yellow-600 rounded-xl">Enroll now</button>
            </div>
          </div>
        </div>
      ))}
